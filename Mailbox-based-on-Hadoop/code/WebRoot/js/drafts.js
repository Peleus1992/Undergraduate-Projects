...
var page_size = 12;
var mail_list;
function addMail(read,file, file_id, to, to_nickname, subject, time, star, mail_id) {
...
function changeMailState(var_mail_id_list, var_to_position, var_from_position,
	...
	$.ajax({  
		...
		success:function(data){  
			...
			if(data.status == "success") {
				window.location.href = "/mailtest/jsp/drafts.jsp";
			} else if(data.status == "failure"){
				alert("Read Mail unsuccessfully!");
			}
			//alert(dat.username + ";" + dat.password);
...
//	}
	...
	$(document).on("click", ".ele_subject", function() {
		...
		window.location.href = "/mailtest/jsp/compose.jsp?mail_id=" + mail_list[i].mail_id
									+"&subject="+mail_list[i].subject+(mail_list[i].file=="true" ? "&file_id="+mail_list[i].file_id
									+"&file_name="+encodeURI(encodeURI(mail_list[i].file_name)) : "")
									+"&to="+mail_list[i].to;
