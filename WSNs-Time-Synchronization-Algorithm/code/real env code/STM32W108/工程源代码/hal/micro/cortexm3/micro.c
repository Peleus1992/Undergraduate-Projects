...
/* Magic number for System bootloader activation, please don't touch */
...
{
  if ((SCS_VTOR == IAP_BOOTLOADER_APPLICATION_ADDRESS) && (mode <= IAP_BOOTLOADER_MODE_OTA)) {
    if (mode ==  IAP_BOOTLOADER_MODE_UART) {
      ...
      if ( (halFixedAddressTable.baseTable.type == FIXED_ADDRESS_TABLE_TYPE) &&
           ( ( (halFixedAddressTable.baseTable.version & FAT_MAJOR_VERSION_MASK) 
               == 0x0000 ) &&
             (halFixedAddressTable.baseTable.version == 0x0003) 
	     ) && (cut >= 2) && (cut <=3 )
	   ) {
        appSwitch->param.offset = blOffset[cut - 2];
      } else {
	/* Cut not supported */
	return ST_ERR_FATAL;
      }
