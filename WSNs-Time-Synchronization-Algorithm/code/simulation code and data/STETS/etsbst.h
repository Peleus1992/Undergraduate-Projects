...
#define TIMER_VALUE 10.0//2.0			// random time
...
class etsbstTimer : public TimerHandler {
	public:
		etsbstTimer(etsbst* agent) : TimerHandler() {
			agent_ = agent;
		}
	protected:
		etsbst* agent_;
...
class roundTimer : public TimerHandler {
	public:
		roundTimer(etsbst* agent) : TimerHandler() {
			agent_ = agent;
			no=0;
		}
	protected:
...
public:
	resetTimer(etsbst* agent) : TimerHandler() {
		agent_ = agent;
	}
protected:
	etsbst* agent_;
...
// The Routing Agent
class etsbst: public Agent {
	friend class etsbst_RTEntry;
	friend class etsbstTimer;
	friend class roundTimer;
	friend class resetTimer;
public:
	etsbst(nsaddr_t id);
...
protected:
	...
	// Packet TX Routines
	void forward(etsbst_RTEntry *rt, Packet *p, double delay);
	...
	bool isRecv;					// the flag to decide whether the node can listen to message;
	// Routing Table
	etsbst_RTable rtable_;

	//Timer
	etsbstTimer etimer;				// timer for decide backbone node
	roundTimer rtimer;				// timer for start synchronization
	resetTimer stimer;				// for the value 'isRecv' to be set to true;
